...
// Вася — уважение (подсечка −1, уговорил не драться +1, рукопожатие +2, извинение +1,
// молча уйти −1).
...
export const chapter1: Record<string, Scene> = {
  ...
  st2: {
    ...
    choices: [
      ...
      {
        text: 'Заговорить ему зубы',
        minutes: 5,
        // драки не будет: Вася сохранит лицо и запомнит это
        check: {
          stat: 'wits',
          difficulty: 1,
          set: { talkedDownVasya: true },
          relation: { vasya: 1 },
          xp: 10,
        },
        next: 'st3_talk',
  ...
  st2_2: {
    ...
    actor: 'img/portrait-vasya.jpg',
    title: 'Не вышло',
    text: '— Слушай, тут все новенькие, давай не будем… — начинаете вы.\n— Умник, да?! — Вася краснеет ещё сильнее. — Сейчас я тебе поумничаю!',
  ...
  st3_talk: {
    ...
    actor: 'img/portrait-vasya.jpg',
    title: 'Без драки',
    text: '— Слушай, — говорите вы негромко, — если нас обоих отправят в карцер в первый же день, кому от этого лучше? Стражники на стене только того и ждут.\nВася косится на стену, где и правда уже собрались зрители, и нехотя опускает кулаки.\n— Ладно… Повезло тебе, новенький. — Он сплёвывает в снег, но злости в голосе уже меньше.\nСо скрипом открываются ворота.',
    choices: [
      { text: 'Направиться к воротам', minutes: 5, next: 'st4' },
      { text: 'Воспользоваться моментом и бежать к лесу', next: 'st1_1' },
    ],
  },
  st2_1: {
    image: 'img/scene-outside-gate.jpg',
  ...
  st5: {
    ...
    text: ({ hero, flag }) =>
      ...
      (flag('talkedDownVasya')
        ? 'Он кивает в сторону ворот, где Вася что-то горячо доказывает стражникам.\n— Видел, как ты уболтал нашего задиру. Язык у тебя подвешен.'
        : 'Он кивает в сторону ворот, где стражники поднимают с земли Васю.\n— Вижу, с местными ты уже познакомился.') +
      ' Здесь не спрашивают, за что тебя привезли: прошлое остаётся за воротами. С сегодняшнего дня ты новобранец Ночной стражи.',
    choices: [
      ...
      return (
        ...
        (flag('sawCloaks')
          ...
          : flag('sawLights')
            ...
            : 'Похоже, ночь, о которой говорил Торвин, всё-таки наступила.') +
        (flag('torvinWarned')
          ? '\nВ ушах звучат слова Торвина: «Не всё, что ходит по тому лесу, — люди».'
          : '') +
        (flag('sawSignal')
          ? '\nВы вспоминаете огонь на угловой башне: три вспышки — и лес ответил.'
          : flag('foundNote')
            ? '\nТри сигнала… «Третья стража — три вспышки», — вспоминаете вы клочок бумаги из казармы.'
            : '') +
